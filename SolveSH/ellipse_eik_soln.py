...
tmax = 500
dt = .1
#sh.solveSH(200*np.pi,50*np.pi,nx,ny,dt,tmax,nsave,"SHEllipse1",
#           Rscale=.5,beta=.45,amplitude=.1,init_flag=3,energy=True)
sh.solveSH(100*np.pi,25*np.pi,nx,ny,dt,tmax,nsave,"SHEllipse2",
           Rscale=.5,beta=.45,amplitude=.1,init_flag=3,energy=True)

#data = sio.loadmat(os.getcwd()+"/data/"+"SHEllipse1.mat")
data = sio.loadmat(os.getcwd()+"/data/"+"SHEllipse2.mat")
U = data['uu']
E = data['ee']
x = data['xx'].T[0,:]
y = data['yy'].T[0,:]
...
#plt.savefig(os.getcwd()+"/figs/"+"SHEllipse1.png")
plt.savefig(os.getcwd()+"/figs/"+"SHEllipse2.png")