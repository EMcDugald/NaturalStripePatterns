...
f = open(os.getcwd()+"/logs/concave_disclination_energy_analysis_v2.out", 'w')
sys.stdout = f
...
def solveSH(Lx,Ly,Nx,Ny,alpha,h,tmax,save_indices,filename,Rscale=.5,rad_scale=.45,amplitude=.5):
    ...
    max_amp = np.zeros(nmax)
    min_amp = np.zeros(nmax)
    total_e = np.zeros(nmax)
    mean_amp = np.zeros(nmax)
    lower_e = np.zeros(nmax)
    lower_e_rat = np.zeros(nmax)
    ...
    for n in range(1, int(nmax) + 1):
       # print("on step:",n)
        ...
        max_amp[n-1] += np.max(u0)
        min_amp[n-1] += np.min(u0)
        total_e[n-1] += np.sum(edensity(xi, eta, u0, ind, R) * dx * dy)
        mean_amp[n-1] += np.mean(np.abs(u0))
        lower_e[n-1] += np.sum(edensity(xi, eta, u0, ind, R) * dx * dy * (mask1 + mask2))
        lower_e_rat[n-1] += lower_e[n-1] / total_e[n-1]
    ...
    ax[0].plot(max_amp)
    ax[0].title.set_text("Max Amplitudes in Time")
    ax[1].plot(min_amp)
    ax[1].title.set_text("Min Amplitudes in Time")
    ax[2].plot(mean_amp)
    ax[2].title.set_text("Mean Amplitudes in Time")
    plt.tight_layout()
    plt.savefig(os.getcwd() + "/figs/energy_analysis_v2/" + figname + "_amp_time_series" + ".png")
    plt.close()

    fig, ax = plt.subplots(nrows=3, ncols=1)
    ax[0].plot(total_e)
    ax[0].title.set_text("Total Energy in Time")
    ax[1].plot(lower_e)
    ax[1].title.set_text("Lower Energy in Time")
    ax[2].plot(lower_e_rat)
    ax[2].title.set_text("Ratio of Lower to Total")
    plt.tight_layout()
    plt.savefig(os.getcwd() + "/figs/energy_analysis_v2/" + figname + "_energy_time_series" + ".png")
    plt.close()
...
for alpha in alphas:
    ...
    plt.savefig(os.getcwd()+"/figs/energy_analysis_v2/"+figname+".png")
    plt.close()
    ...
    ax[0, 1].title.set_text("Field cross section along y=0")
    ax[1, 1].plot(E[int(ny/2),:, 2])
    ax[1, 1].title.set_text("Energy cross section along y=0")
    #fig.subplots_adjust(wspace=.2, hspace=.2)
    plt.tight_layout()
    plt.savefig(os.getcwd() + "/figs/energy_analysis_v2/" + figname + "_profiles"+".png")
    plt.close()
