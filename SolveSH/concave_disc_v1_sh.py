...
def solveSH(Lx,Ly,Nx,Ny,h,tmax,save_indices,filename,Rscale=.5,beta=.4,amplitude=.9,angle_fctr=3.0,init_phase_scale=1.):
    '''
    ...
    # set R function
    theta = (2. / angle_fctr) * (np.sqrt(X ** 2 + Y ** 2)) ** (angle_fctr / 2.) * np.sin(
        (angle_fctr / 2.) * np.arctan2(X, Y))
    mask = np.tanh(np.sqrt(Lx**2+Ly**2)*(beta-np.sqrt((X/Lx)**2+(Y/Ly)**2))/2)+1
    R = Rscale*(mask-1)

    # set initial condition
    u0 = np.cos(init_phase_scale*theta)*amplitude*.5*mask
...
angle_factor = 3.3 #1-2 gives pgb like structure, 2-3 gives concave disc of narrow to equal aspect, 3+ gives concave disc of wider aspect
init_phase_scale = .1 #controls how many stripes are in init. increasing this gives more stripes
figname = "SHCncvDisc_v1_alpha_33em1_ps_1em1"

solveSH(60*np.pi,60*np.pi,nx,ny,dt,tmax,save_indices,figname,
        beta=.4,amplitude=.8,
        angle_fctr=angle_factor,
        init_phase_scale=init_phase_scale)
...
ax[1,0].imshow(U[:,:,5],cmap='gray')
ax[1,0].title.set_text('t={}: full field'.format(5*dt))
ax[1,1].imshow(U[y_st:y_end,x_st:x_end,5],cmap='gray')
ax[1,1].title.set_text('t={}: interier field'.format(5*dt))
ax[1,2].imshow(E[y_st:y_end,x_st:x_end,5],cmap='gray')
ax[1,2].title.set_text('t={}: interier energy'.format(5*dt))

ax[2,0].imshow(U[:,:,10],cmap='gray')
ax[2,0].title.set_text('t={}: full field'.format(10*dt))
ax[2,1].imshow(U[y_st:y_end,x_st:x_end,10],cmap='gray')
ax[2,1].title.set_text('t={}: interier field'.format(10*dt))
ax[2,2].imshow(E[y_st:y_end,x_st:x_end,10],cmap='gray')
ax[2,2].title.set_text('t={}: interier energy'.format(10*dt))

ax[3,0].imshow(U[:,:,20],cmap='gray')
ax[3,0].title.set_text('t={}: full field'.format(20*dt))
ax[3,1].imshow(U[y_st:y_end,x_st:x_end,20],cmap='gray')
ax[3,1].title.set_text('t={}: interier field'.format(20*dt))
ax[3,2].imshow(E[y_st:y_end,x_st:x_end,20],cmap='gray')
ax[3,2].title.set_text('t={}: interier energy'.format(20*dt))

ax[4,0].imshow(U[:,:,50],cmap='gray')
ax[4,0].title.set_text('t={}: full field'.format(50*dt))
ax[4,1].imshow(U[y_st:y_end,x_st:x_end,50],cmap='gray')
ax[4,1].title.set_text('t={}: interier field'.format(50*dt))
ax[4,2].imshow(E[y_st:y_end,x_st:x_end,50],cmap='gray')
ax[4,2].title.set_text('t={}: interier energy'.format(50*dt))

print("total energy at time {}: ".format(0*dt), np.sum(E[:,:,0]*dx*dy))
print("total energy at time {}: ".format(5*dt), np.sum(E[:,:,5]*dx*dy))
print("total energy at time {}: ".format(10*dt), np.sum(E[:,:,10]*dx*dy))
print("total energy at time {}: ".format(20*dt), np.sum(E[:,:,20]*dx*dy))
