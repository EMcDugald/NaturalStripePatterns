...
#########################################################################################################################
    ### Same as v1, but just optimizing in k11 and k12 (since k21, k22 are determined from them)
    ### also, hardcoding a parameter for now
#########################################################################################################################
...
logfile = open(os.getcwd()+"/logs/sh_pgbs/wv_gen_sh_kb_v2_mu_{}.out".format(mu), 'w')
sys.stdout = logfile
...
def grad_obj(A1,k11,k12,phi):
    """
    gradient of objective function
    """
    do_dA1 = np.mean(
        2 * (A1 * np.cos(phi - 1.0 * np.log(2 * np.exp(X * k11) * np.cosh(Y * k12))) - W) * np.cos(
            phi - 1.0 * np.log(2 * np.exp(X * k11) * np.cosh(Y * k12)))
    )
    do_dk11 = np.mean(
        2.0 * A1 * X * (A1 * np.cos(phi - 1.0 * np.log(2 * np.exp(X * k11) * np.cosh(Y * k12))) - W) * np.sin(
            phi - 1.0 * np.log(2 * np.exp(X * k11) * np.cosh(Y * k12)))
    )
    do_dk12 = np.mean(
        -2.0 * A1 * Y * (A1 * np.cos(phi - 1.0 * np.log(2 * np.exp(X * k11) * np.cosh(Y * k12))) - W) * (
                    np.exp(X * k11 - Y * k12) - np.exp(X * k11 + Y * k12)) * np.sin(
            phi - 1.0 * np.log(2 * np.exp(X * k11) * np.cosh(Y * k12))) / (
                    np.exp(X * k11 - Y * k12) + np.exp(X * k11 + Y * k12))
    )
    do_dphi = np.mean(
        -2 * A1 * (A1 * np.cos(phi - 1.0 * np.log(2 * np.exp(X * k11) * np.cosh(Y * k12))) - W) * np.sin(
            phi - 1.0 * np.log(2 * np.exp(X * k11) * np.cosh(Y * k12)))
    )
    return np.array([do_dA1,do_dk11,do_dk12,do_dphi])
...
if print_grad:
    import sympy as sp
    A1sym, k11sym, k12sym, xsym, ysym, wsym, phisym = sp.symbols('A1,k11,k12,X,Y,W, phi')
    obj_fn = (A1sym*sp.cos(a*sp.log(sp.exp(k11sym*xsym + k12sym*ysym)
                                       +sp.exp(k11sym*xsym - k12sym*ysym)) - phisym) - wsym)**2
    phase_fn = k11sym*xsym + sp.log(2*sp.cosh(k12sym*ysym))
    print("Objective Function:", obj_fn)
    ...
    [print("deriv in {}".format(str(x)),"\n",str(sp.simplify(obj_fn.diff(x))).
           ...
           .replace("cos","np.cos").replace("sin","np.sin"))
            for x in [A1sym,k11sym,k12sym,phisym]]
    if print_hess:
        print("Objective Function all hessian terms:")
        [[print(str(sp.simplify(obj_fn.diff(x).diff(y)))) for x in [A1sym,k11sym, k12sym,phisym]]
         for y in [A1sym,k11sym,k12sym,phisym]]
...
max_its = 10000
i = 0
print("Init Vals:",A10, k110, k120, phi0)
while np.linalg.norm(grad_obj(A10,k110,k120,phi0))>1e-6 and i < max_its:
    curr = np.array([A10,k110,k120,phi0])
    ...
    while obj(new[0],new[1],new[2],new[3])>obj(curr[0],curr[1],curr[2],curr[3]):
        ...
        if d<1e-16:
            ...
            print("Obj Function Norm: ", np.linalg.norm(obj(A10,k110,k120,phi0)))
            print("New Vals: ", A10, k110, k120, phi0)
            break
    A10, k110, k120, phi0 = new
    i += 1
    print("Step: ", i)
    print("Gradient Norm", np.linalg.norm(grad))
    print("Obj Function Norm: ",np.linalg.norm(obj(A10,k110,k120,phi0)))
...
plt.tight_layout()
plt.savefig(os.getcwd()+"/figs/sh_pgbs/FieldEst_v2_mu_{}.png".format(mu))
...
plt.tight_layout()
plt.savefig(os.getcwd()+"/figs/sh_pgbs/PhaseEst_v2_mu_{}.png".format(mu))
