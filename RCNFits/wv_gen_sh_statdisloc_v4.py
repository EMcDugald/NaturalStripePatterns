...
def grad_obj_fd(kb,b,phi,X,Y,W):
    ...
    """
    do_dkb = np.mean(
        (obj(kb+diff,b,phi,X,Y,W)-obj(kb,b,phi,X,Y,W))/diff
    )
    do_db = np.mean(
    ...
    return np.array([do_dkb,do_db,do_dphi])
...
while np.linalg.norm(grad_obj_fd(kb0,beta0,phi0,Xhalf,Yhalf,Whalf))>1e-4 and i < max_its:
    curr = np.array([kb0,beta0,phi0])
    grad = grad_obj_fd(curr[0],curr[1],curr[2],Xhalf,Yhalf,Whalf)
    d = step
    new = curr - d*grad
    while obj(new[0],new[1],new[2],Xhalf,Yhalf,Whalf)>obj(curr[0],curr[1],curr[2],Xhalf,Yhalf,Whalf):
...
plt.suptitle("Pattern, Approx Pattern, and Error")
plt.tight_layout()
plt.savefig(os.getcwd()+"/figs/sh_dislocation/FieldEst_v1.png")
