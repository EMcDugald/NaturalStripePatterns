...
while np.linalg.norm(grad_obj_fd(kb0,beta0,phi0,Xhalf,Yhalf,Whalf))>1e-8 and i < max_its:
    curr = np.array([phi0])
    grad = grad_obj_fd(kb0,beta0,curr[0],Xhalf,Yhalf,Whalf)
    d = step
    new = curr - d*grad
    while obj(kb0,beta0,new[0],Xhalf,Yhalf,Whalf)>obj(kb0,beta0,curr[0],Xhalf,Yhalf,Whalf):
