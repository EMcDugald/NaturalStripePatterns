...
dir = os.getcwd()+"/data/"
sio.savemat(dir+"mu={:.3f}".format(mu)+".mat", mdict)

fig, ax = plt.subplots(nrows=1,ncols=1)
#ax.imshow(uu[int(3*Ny/16):int(5*Ny/16),:,-1])
ax.imshow(uu[int(7*Ny/32):int(9*Ny/32),int(Nx/4):int(3*Nx/4),-1])
ax.title.set_text('t={:.3e}'.format(tt[-1]))
plt.tight_layout()
plt.savefig(os.getcwd() + "/figs/mu={:.3f}.png".format(mu))
